...
public class Ex112 {
    public int[] solution(int m, int n, int startX, int startY, int[][] balls) {
        ...
        // 시작점을 y축, x축, x=m, y=n에 대하여 대칭시킨 좌표
        int[] dx = {-startX, startX, m*2-startX, startX};
        int[] dy = {startY, -startY, startY, n*2-startY};

        for (int i=0; i<answer.length; i++) {
            if (startX == balls[i][0]) {
                int x = Math.min(m-startX, startX) * 2;
                int y = startY - balls[i][1];
                int temp = 0;
                if (startY < balls[i][1]) {
                    temp = startY + balls[i][1];
                } else {
                    temp = n * 2 - startY - balls[i][1];
                }
                answer[i] = Math.min(x * x + y * y, temp * temp);
            } else if (startY == balls[i][1]) {
                int x = startX - balls[i][0];
                int y = Math.min(n-startY, startY) * 2;
                int temp = 0;
                if (startX < balls[i][0]) {
                    temp = startX + balls[i][0];
                } else {
                    temp = m * 2 - startX - balls[i][0];
                }
                answer[i] = Math.min(x * x + y * y, temp * temp);
            } else {
                int temp = Integer.MAX_VALUE;
                for (int j=0; j<4; j++) {
                    int x = balls[i][0] - dx[j];
                    int y = balls[i][1] - dy[j];
                    temp = Math.min(temp, x*x + y*y);
                }
                answer[i] = temp;
            }
        }
