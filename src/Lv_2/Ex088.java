...
public class Ex088 {
    ...
    int[][] prices; // 할인 적용해서 가격을 담을 배열

    public int[] solution(int[][] users, int[] emoticons) {
        prices = new int[emoticons.length][4];

        // 10, 20, 30, 40 할인 적용해서 배열에 담기
    ...
    void dfs(int[][] users, int[] emoticons, int index, int[] sum) {
        if (index == emoticons.length) { // 종료 지점
            int[] result = new int[2];

            // 일정 가격 이상일 때 임티플로 전환
            for (int i=0; i<users.length; i++) {
                if (users[i][1] <= sum[i]) result[0]++;
                else result[1] += sum[i];
            }

            // 임티플 회원이 더많거나 수가 같으면서 가격의 합이 더 클 때
        ...
        for (int i=0; i<4; i++) {
            // 같은 배열을 참조하지 않도록 초기화
            int[] arr = Arrays.copyOf(sum, sum.length);

            // 할인율에 따라 사거나 사지 않음
            for (int j=0; j<users.length; j++) {
                if (users[j][0] <= (i+1)*10) {
                    arr[j] += prices[index][i];
                }
            }

            dfs(users, emoticons, index+1, arr);
        }
    }
}
