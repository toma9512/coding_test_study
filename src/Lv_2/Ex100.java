...
public class Ex100 {
    ...
    public void dfs(int n, int[] info, int[] arr, int index, int apeach, int rion) {
        if (index == 10) {
            //System.out.println(rion-apeach);
            //System.out.println(apeach);
            if (rion > apeach) {
                // System.out.println(Arrays.toString(arr));
                if (score < rion - apeach) {
                    arr[10] = n;
                    score = rion - apeach;
                    result = arr;
                } else if (score == rion - apeach) {
                    for (int i=10; i>=0; i--) {
                        if (result[i] < arr[i]) {
                            result = arr;
                            break;
        ...
        dfs(n, info, copy, index+1, num, rion);

        int[] copy1 = Arrays.copyOf(arr, 11);
        if (info[index] < n) {
            copy1[index] = info[index] + 1;
            dfs(n - info[index] - 1, info, copy1, index+1, apeach, rion+10-index);
        }
    }
}
