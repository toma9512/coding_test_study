...
public class Ex049 {
    public String[] solution(String[] files) {
        ...
        for (int i=0; i<files.length; i++) {
            char[] chArr = files[i].toCharArray();
            int index = 0;
            sb.setLength(0);
            ...
            for (char ch : chArr) {
                if (index == 0) { // head일 때
                    if (ch<='9' && ch>='0') {
                        arr[i][index++] = sb.toString();
                        sb.setLength(0);
                    }
                } else if (index == 1) { // number일 때
                    if (ch>'9' || ch<'0') {
                        arr[i][index++] = sb.toString();
                        sb.setLength(0);
                    }
            ...
            arr[i][index] = sb.toString(); // tail
            if (arr[i][2] == null) arr[i][2] = "";
        }
        ...
        Arrays.sort(arr, (o1, o2) -> {
            ...
            if (word1.equals(word2)) { // head가 같을 때
