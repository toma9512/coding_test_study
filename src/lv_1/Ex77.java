...
public class Ex77 {
    public int[] solution(String today, String[] terms, String[] privacies) {
        ...
        for (int i=0; i<privacies.length; i++) {
            // 약관 종류 찾기
            String word = privacies[i].substring(11);
            int plus = 0;
            for (String term : terms) {
                if (term.contains(word)) {
                    plus = Integer.parseInt(term.substring(2));
                    break;
                }
            }
            ...
            if (todayYear > year) list.add(i+1);
            else if (todayYear==year && todayMonth>month) list.add(i+1);
            else if (todayYear==year
                    && todayMonth==month
                    && todayDay>=day) list.add(i+1);
        }
